...
set _DeploySetParametersFile=%RootPath%BlutzuckerConverter.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%BlutzuckerConverter.zip" (
echo "%RootPath%BlutzuckerConverter.zip" does not exist. 
...
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%BlutzuckerConverter.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -skip:objectname='dirPath',absolutepath='obj\\Release\\Package\\PackageTmp\\App_Data$' -skip:objectname='dirPath',absolutepath='BlutzuckerConverter\\App_Data$'
) else (
...
:usage
echo =========================================================
if not exist "%RootPath%BlutzuckerConverter.deploy-readme.txt" (
...
start notepad "%RootPath%BlutzuckerConverter.deploy-readme.txt"
)
echo =========================================================
goto :eof
